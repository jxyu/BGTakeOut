...
//    }
    ...
    UIScrollView * img_scrollview=[[UIScrollView alloc] initWithFrame:CGRectMake(0, 64, SCREEN_WIDTH, SCREEN_HEIGHT-64)];
    for (int i=0; i<dict.count; i++) {
        //        [imgView sd_setImageWithURL:[NSURL URLWithString:@"http://cc.cocimg.com/api/uploads/20150430/1430388577515100.jpg"] placeholderImage:[UIImage imageNamed:@"1136-1"]];＋＋＋＋＋＋＋＋＋＋＋＋＋＋＋＋＋＋＋＋＋
        //!!!: 图片显示使用上面的链接可以正常显示，使用服务器返回的图片链接，错误 -1100  ，怀疑是使用外链进行多次跳转，服务器没有设置商铺相册对应的模块
        UIImageView* imgView=[[UIImageView alloc] initWithFrame:CGRectMake(0,i*250, SCREEN_WIDTH, 240)];
        [imgView sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"http://112.74.76.91/baguo/%@",[[dict objectAtIndex:i] objectForKey:@"picurl"]]] placeholderImage:[UIImage imageNamed:@"1136-1"]];
        [img_scrollview addSubview:imgView];
    }
    UIView * lastview=[img_scrollview.subviews lastObject];
    img_scrollview.contentSize=CGSizeMake(0, lastview.frame.size.height+lastview.frame.origin.y+10);
    [self.view addSubview:img_scrollview];
